...
class MediaAdmin extends Component {
    ...
    selectFile = (file_data) => {
        console.log(":: MediaApp - selected file: ", file_data);
                    ...
                    if (units.total > 0) {
                        ...
                        insertName(sha1, "sha1", (data) => {
                            if (data.length > 0 && file_data.file_name !== data[0].file_name) {
                                ...
                                file_data.line.fix_media_id = data[0].send_id;
                        ...
                        mediaName(file_data.file_name, (data) => {
                            if(data.length > 1 && data[data.length-2].sha1 !== file_data.sha1) {
                                file_data.line.old_sha1 = data[data.length-2].sha1;
                                file_data.line.fix_media_id = data[data.length-2].media_id;
                            ...
                            } else if(data.length > 1 && data[data.length-2].sha1 === file_data.sha1) {
    ...
    newInsertData = (file_data, insert_old, insert_mode) => {
        if (file_data.line.content_type) {
            ...
            insert_data.send_id = file_data.media_id;
            insert_data.send_uid = insert_mode === "3" ? insert_old[0].line.uid : "";
            insert_data.upload_type = "media";
            insert_data.sha1 = file_data.sha1;
            insert_data.size = parseInt(file_data.size, 10);
    ...
    setMeta = (insert_data) => {
        ...
        file_data.extra = {insert_id: insert_data.insert_id, name: insert_data.line.send_name};
        ...
        // TODO: media backend - /workflow/insert media flow not verified yet
        putData(`${WFSRV_BACKEND}/workflow/insert`, insert_data, (cb) => {
            ...
            if(cb.status === "ok") {
                ...
                putData(`${WFDB_BACKEND}/media/${file_data.media_id}`, file_data, (cb) => {
    ...
    renameFile = (newline) => {
        ...
        let opath = `/backup/media/${newfile_name}_${file_data.media_id}o.mp4`;
        file_data.line = {...newline};
        file_data.extra = {...{file_name: oldfile_name}};
        //file_data.line.title = this.state.tags[newline.pattern] || "";
        file_data.file_path = opath;
        ...
        // TODO: media backend - /workflow/rename media flow not verified yet
        putData(`${WFSRV_BACKEND}/workflow/rename`, file_data, (cb) => {
            console.log(":: Media - rename respond: ",cb);
            if(cb.status === "ok") {
    ...
    sendFile = () => {
        ...
        // TODO: media backend - /workflow/send_media endpoint does not exist yet
        putData(`${WFSRV_BACKEND}/workflow/send_media`, file_data, (cb) => {
            console.log(":: Media - send respond: ",cb);
    ...
    setRemoved = () => {
        ...
        fetch(`${WFDB_BACKEND}/media/${file_data.media_id}/wfstatus/removed?value=true`, { method: 'POST',headers: {'Authorization': 'bearer ' + getToken()}})
    };
    ...
    render() {
        ...
        let media = this.props.media.map((data) => {
            const {backup,kmedia,metus,source,removed,wfsend,censored,checked,fixed,locked} = data.wfstatus;
            let id = data.media_id;
            // TODO: media has no "proxy" field like aricha - treat ready by extra.proxy or file_path presence
            let ready = data.extra?.proxy ?? !!data.file_path;
        ...
        return (
            <Segment textAlign='center' className="ingest_segment" color='brown' raised>
                <Label  attached='top' className="trimmed_label">
                    {file_data.file_name ? file_data.file_name : "Media Status"}
                </Label>
                {kc.hasRealmRole("wf_media") ?
                    <Message>
