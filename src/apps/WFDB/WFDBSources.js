...
class Sources extends Component {
    ...
    componentDidMount() {
        this.getSourcesData("wfdb", "date", this.props.date);
    };

    getSourcesData = (sjson, skey, svalue) => {
        let search = this.props.skey === "date" && !svalue ? this.props.date : svalue;
        let endpoint = sjson === "wfdb" ? "find" : skey === "sha1" ? "sha1" : sjson;
        if(!search) return;
        getData(`source/${endpoint}?key=${skey}&value=${search}`, (sources) => {
            console.log(":: Sources DB Data: ",sources);
    ...
    searchData = (tab) => {
        ...
        if(!search) return;
        getData(`${tab}/${endpoint}?key=${skey}&value=${search}`, (sources) => {
            console.log(":: Sources DB Data: ",sources);
            this.setState({sources});
        });
    };
    ...
    getStatus = (data) => {
        ...
        this.setState({wfstatus: {...data.wfstatus}, id: data.source_id})
    };

    getLine = (data) => {
        console.log(":: Got status: ",data);
        this.setState({line: {...data.line}, id: data.source_id, value: data.line.week_date})
    };
    ...
    setLine = () => {
        ...
        postData(`${WFDB_BACKEND}/source/${id}/line`, line, (cb) => {
    ...
    toggle = (data) => {
        ...
        fetch(`${WFDB_BACKEND}/source/${this.state.id}/wfstatus/${data}?value=${wfstatus[data]}`, { method: 'POST',headers: {'Authorization': 'bearer ' + getToken()}});
    ...
    render() {
        ...
        let trimmer_data = this.state.sources.map((data) => {
